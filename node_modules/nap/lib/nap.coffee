...
module.exports.js = (pkg, gzip = @gzip) =>
  ...
  for filename, contents of preprocessPkg pkg, 'js'
    ...
    output += "<script src='#{@_assetsDir}/#{filename}?#{fingerprint ? ''}' type='text/javascript'></script>"
  output
...
module.exports.css = (pkg, gzip = @gzip) =>
  ...
  for filename, contents of preprocessPkg pkg, 'css'
    ...
    output += "<link href='#{@_assetsDir}/#{filename}?#{fingerprint ? ''}' rel='stylesheet' type='text/css'>"
  output
